...
from .common import (
    ...
    chunk_jobs, jobs_prompt, normalize_items,
)
...
def rank_stage(workflow: str, candidate: Candidate, jobs: list[Job],
    ...
    """Shared B0/B1 map-reduce ranking. Returns ranked items (best first)."""
    ...
    for chunk in chunk_jobs(jobs):
        prompt = jobs_prompt(candidate, chunk, extra=extra_context) + (
            f"\n\nVelg de inntil {CHUNK_KEEP} beste stillingene fra listen over "
            "for denne kandidaten. Sett score 0–100."
        )
        result = client.complete(task="v2.rank_chunk", system=_SYSTEM, user=prompt,
                                 json_schema=RANK_SCHEMA, max_tokens=6000, tags=tags)
    ...
    prompt = jobs_prompt(candidate, finalist_jobs, extra=extra_context) + (
        "\n\nDette er finalistene fra en grovsortering. Gi den ENDELIGE "
        "rangeringen: inntil 50 stillinger, best først, med score 0–100 og "
        "belagte påstander. Ta bare med stillinger som faktisk er verdt å søke på."
    )
